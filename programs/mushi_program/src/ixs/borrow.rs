...
use crate::{
    constants::{
        ...
        add_loans_by_date, burn_tokens, get_interest_fee, get_midnight_timestamp, liquidate, transfer_tokens, transfer_tokens_checked
    }
};
...
pub fn borrow(ctx:Context<ACommonExtLoan>, number_of_days: u64, es_amount:u64)->Result<()>{
    ...
    ctx.accounts.common.safety_check_borrow(new_user_borrow - eclipse_fee + fee_address_fee, false, user_mushi)?;
    Ok(())
}
